use crate::{
    ...
    core::{
        ...
        constants::{
            MAX_EXCLUDE_PATHS, MAX_EXCLUDE_PATH_LENGTH, MAX_EXTENSIONS, MAX_QUERY_LENGTH, MAX_ROOTS,
        },
        content_search::{ContentSearchResponse, ContentSearchService},
...
pub fn search_start(
...

    {
        let mut thread_handles =
            crate::lock_mutex!(state.search_thread_handles, "search_thread_handles");
...
fn terminal_event_from_stream(
    ...
    match outcome {
        ...
        Ok(summary) if summary.cancelled => {
            SearchTerminalEvent::Cancelled(SearchCancelledEvent { search_id })
        }
...
fn format_search_error(message: &str) -> (String, String) {
    ...
    } else if message.contains("index store lock poisoned")
        || message.contains("search session lock poisoned")
    {
        "SEARCH_STATE_ERROR"
    } else {
    ...
    match request.options.search_backend {
        ...
        SearchBackend::Index => {
            ...
            if index_snapshot.entries.is_empty() {
                ...
                let summary =
                    IndexService::stream(&index_snapshot, request, cancel_flag, emit_batch, |_| {})
...
async fn enrich_paths_with_metadata(
    paths: &[String],
...
mod tests {
    ...
    fn format_search_error_uses_consistent_codes() {
        assert_eq!(
            ...
            (
                "SEARCH_INVALID_QUERY".to_string(),
                "regex parse error: [".to_string()
            )
        );
        assert_eq!(
            format_search_error("search session lock poisoned"),
            (
                "SEARCH_STATE_ERROR".to_string(),
                "search session lock poisoned".to_string()
            )
        );
        assert_eq!(
    ...
    async fn enrich_paths_with_metadata_populates_entries() {
        ...
        let items = enrich_paths_with_metadata(&paths, &roots)
            .await
            .expect("metadata");
...
pub async fn content_search(
    ...
    let result = tokio::task::spawn_blocking(move || {
        ContentSearchService::search_in_content(
            &paths,
            &query,
            case_sensitive,
            whole_word,
