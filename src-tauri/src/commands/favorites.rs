...
mod tests {
    ...
    fn favorites_inner_commands_flow() {
        with_test_data_dir(|| {
            ...
            assert_eq!(
                favorites_list_inner(&favorites).expect("list"),
                Vec::<String>::new()
            );

            let added = favorites_add_inner(&favorites, path_one.to_string_lossy().to_string())
                .expect("add");
            assert_eq!(added, vec![path_one.to_string_lossy().to_string()]);
            assert!(!favorites_remove_inner(
                &favorites,
                path_missing.to_string_lossy().to_string()
            )
            .expect("remove missing"));
            assert!(
                favorites_remove_inner(&favorites, path_one.to_string_lossy().to_string())
