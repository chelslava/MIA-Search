...
impl IndexSnapshot {
    ...
    pub fn with_limits(roots: Vec<String>, entries: Vec<SearchResultItem>) -> (Self, usize, bool) {
        Self::with_limits_impl(
            roots,
            entries,
            MAX_INDEX_ENTRIES,
            MAX_INDEX_SIZE_MB * 1024 * 1024,
        )
    }

    #[cfg(test)]
    pub fn with_limits_with_max_entries(
        roots: Vec<String>,
        entries: Vec<SearchResultItem>,
        max_entries: usize,
    ) -> (Self, usize, bool) {
...
mod tests {
    ...
    fn with_limits_returns_all_entries_when_within_limits() {
        ...
        let (snapshot, total_size, truncated) =
            IndexSnapshot::with_limits(vec!["/".to_string()], entries.clone());
    ...
    fn with_limits_truncates_at_entry_count() {
        ...
        let (snapshot, _, truncated) =
            IndexSnapshot::with_limits_with_max_entries(vec!["/".to_string()], entries, 5);
    ...
    fn estimated_memory_bytes_matches_individual_estimates() {
        let entries: Vec<SearchResultItem> =
            vec![make_item("/a.txt", "a.txt"), make_item("/b.txt", "b.txt")];
