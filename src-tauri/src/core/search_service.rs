...
fn sort_stream_batch(items: &mut [SearchResultItem], mode: &SortMode, query: &str) {
    match mode {
        SortMode::Relevance if query.is_empty() => {
...
mod tests {
    ...
    fn multi_extension_dedupe_behavior_basic_sanity() {
        ...
        let execution = SearchService::execute(&request);
        let paths: HashSet<String> = execution
            .items
            .iter()
    ...
    fn depth_restriction_basic_case() {
        ...
        let execution = SearchService::execute(&request);
        let names: HashSet<String> = execution
            .items
            .iter()
        ...
        assert!(!execution
            .items
            .iter()
            .any(|item| item.full_path.ends_with("nested/deep.txt")));
    }

    #[test]
    fn query_matcher_plain_respects_case_flag() {
        let case_sensitive = build_query_matcher(&MatchMode::Plain, "Report", false)
            .expect("plain matcher should compile");
        let ignore_case = build_query_matcher(&MatchMode::Plain, "report", true)
            .expect("plain matcher should compile");
    ...
    fn query_matcher_wildcard_handles_xls_pattern() {
        let matcher = build_query_matcher(&MatchMode::Wildcard, "*.xls*", true)
            .expect("wildcard matcher should compile");
    ...
    fn permission_denied_root_does_not_fail_stream() {
        ...
        assert!(
            result.is_ok(),
            "stream should degrade gracefully on permission denied"
        );
    }
