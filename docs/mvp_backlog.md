...
- [x] Backend режимы `Scan/Index`, где `Scan` включён по умолчанию.
...
- [x] Быстрые toggle-кнопки режимов под полем поиска + контекстные подсказки.
...
- [x] Финальный Windows release smoke (`cargo tauri build` + чек-лист).

## 6. Документация и релиз
- [x] Финализировать пользовательскую инструкцию запуска и сценарии поиска.
- [x] Зафиксировать MVP release notes.
...
- Инкрементальная/фоновая индексация и шедулинг политик обновления.
